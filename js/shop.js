...
const totalAddCartItemsCount = document.getElementById('totalCartItemsCount');
const totalAddWishListItemsCount = document.getElementById(
...
async function addToCart(productId) {
    ...
    if (shoppingCartList.some((product) => product.id === productId)) {
    } else {
    ...
    totalAddCartItemsCount.innerHTML = shoppingCartList.length;
}
const shoppingWishlistItems = [];
async function addToWishList(productId) {
    //if the product alredy exist
    const products = await fetchData();
    if (shoppingWishlistItems.some((product) => product.id === productId)) {
    } else {
        const product = products.find((product) => product.id === productId);
        console.log('product', product);
        shoppingWishlistItems.push({
            ...product,
            numberOfUnits: 1,
        });

        totalAddWishListItemsCount.innerHTML = shoppingWishlistItems.length;
        // updateWishList();
        // checkoutButtonVisibility();
    }
}
...
//     console.log('shopping cart items', shoppingCartList);
